...
因为我们会在书中大量应用系统思考来分析指导组织设计选择和变革策略，所以本章试图给大家做些必要的准备。但这不是一本全面介绍系统思考的书，除了阐述最基础的概念外，我们会结合产品开发组织相关的例子来帮助大家快速地看到系统思考和大规模产品开发组织这个话题的相关性，同时重点分享自己在实践系统思考过程中形成的一些理解和洞察。

##1. 系统思考

本书的副标题是“用系统思考指导组织设计和变革”。为什么用系统思考？Peter Senge在《第五项修炼》中区分定义了细节复杂度和动态复杂度。细节复杂度以涉及变量多为其特征，而动态复杂度却以因果关系之间的微妙为其特征。因和果经常不在同一时间和空间带来了高的动态复杂度，而系统思考能帮助我们更好地理解这样的问题并进行有效干预。大规模产品开发组织的设计和变革不仅是细节复杂的，还是动态复杂的，因此适合用系统思考来分析并指导。

系统思考本身的定义也有一定的模糊性，我们书中采用的概念和工具都来自于Jay Forrester奠基的系统动力学。系统动力学包含的主要工具有四个：因果回路图（Causal-Loop Diagram）、行为模式曲线（Behavior over Time Diagram）、存量流量图（Stock & Flow Diagram）和计算机模拟（Computer simulation)。系统思考的很多洞察是反直觉的，而基于计算机模拟的定量分析能够让我们产生新的洞察并改变我们的心智模式。但是就组织设计和变革的话题通过数学建模进行定量分析并不成熟，同时相信基于因果回路图的定性分析对于我们改变心智模式（对因果关系的思考从线性转向回路）已经能有很大的推动，所以本书将以因果回路图作为系统思考的建模和分析工具。

##2. 因果回路图（CLD：Causal-Loop Diagram）

因果回路图在一般的系统思考书（比如《系统思考》和《Systems Thinking Basics》）里都有介绍。它基本的要素有：变量、链路和回路，我们接下来对这些要素逐一介绍。
...
变量之间可以形成链路，这个链路是因果链路。变量A到变量B的链路是指，假设其它因素都一样的情况下，A的变化导致B的变化。这个变化可以是正向的，以“+”标注；也可以是负向的，以“-”标注。链路上可以有延迟，以“||”标注。

我们通过两个例子来说明。

####1. “需求量”和“人数”之间的链路

“需求量”变多，“人数”是变多还是变少，还是两者之间没有关联（也就是不形成链路）？一种解释是，“需求量”变多，“人数”变多，因为我们会招更多人做需求。这样就在“需求量”和“人数”之间形成了正向的链路。
...
“招聘力度”变大，“实际人数”变多；那么“招聘力度”变小，“实际人数”变少吗？只要是在招聘，实际人数是不会变少的。所以这里用同向/反向的表述有些难以理解，而正向/反向的表述则更为恰当。正向/负向究竟是什么含义呢？准确的说法应该是，“招聘力度”变小，“实际人数”相比按之前的招聘力度能获得的实际人数变得更小。因此无论从哪个变化的方向看，它都是正向的关系。

####2. “加班量”和“产出”之间的链路

“加班量”和“产出”之间有链路吗？是正向还是负向的？一种解释可以是，“加班量”变多，“工作时长”变长，“产出”变多。这是正向的链路。

![](Link2.1.jpg)

而另一种解释可以是，“加班量”变多，“士气”变低，“效率”变低，“产出”变少。

![](Link2.2.jpg)

“加班量”和“士气”之间是负向的链路。这个负向链路也使得从“加班量”到“产出”整体也是负向的，因为其余的链路都是正向的。要判断两个变量之间最终的因果关系，只需数其中负向链路的个数，如果是偶数个，就是正向的；如果是奇数个，就是负向的。
...
链路可能形成回路。从变量A到变量B有一条链路，当变量B的变化反过来又导致变量A的变化（也就是从变量B到变量A也有一条链路）时，它们就形成了回路。有两种回路，一种是增强回路（Reinforcing loop，标注为R），另一种是平衡回路（Balancing loop，标注为B）。
...
需要注意的是，我上面把它读成了一个良性循环，我也可以从另一个方向读它。“价值”变低，“销量”变少，“利润”变低，“投入”变少，“价值”变低，由此形成的还是增强回路，但它却是个恶性循环。所以增强回路的特征在于增强，它可以是良性或是恶性。

当形成回路时，给回路命名是重要的思考步骤。回路只是更大动态里的一部分，通过给回路命名，我们就能在更高的抽象层次看待理解整个动态。以刚才的增强回路为例，我们可以把它命名为“价值驱动产品增长”。

###2. 平衡回路（B）

我们继续展开之前“需求量”和“人数”之间的链路，“实际人数”变多，完成的“需求量”变多。。。这时我们发现“需求量”这个变量定义得并不准确，可以把它区分定义为三个与“需求量”相关但又不同的变量 - “完成需求量”、“待做需求量”和“输入需求量”。“待做需求量”是“输入需求量”和“完成需求量”的差值。“输入需求量”变多，“待做需求量”变多，它们之间是正向链路；“完成需求量”变多，“待做需求量”变少，它们之间是负向链路。

![](LoopB1.jpg)

“待做需求量”变多，“需要人数”变多，“招聘力度”变大，“实际人数”变多，“完成需求量”变多，“待做需求量”变少，由此形成了一个平衡回路。我们可以把它命名为“招人完成需求”。

我们平时的生活中也有恶性循环/良性循环这样的说法，所以在我的经验里掌握增强回路并不困难。更难掌握的反而是平衡回路，因为平衡回路的特征是寻求目标的达成。我们可以把问题定义为目标和现状的差距，这个差距驱动行为的改变，从而导致现状发生变化，差距得以缩小。
...
是增强回路还是平衡回路的判断除了按部就班逐个变量和链路演绎之外，还可以数回路里包含负向链路的个数，是偶数个就是增强回路，是奇数个就是平衡回路。给回路命名的步骤可以作为一个检验，当你发现给增强回路的命名是解决问题/达成目标之类，而给平衡回路的命名是增长/衰退之类，就该再仔细查看一遍了。
...
“舍本逐末”系统基模由两个平衡回路和一个增强回路组成。如果“走捷径以加速”的平衡回路（上图左上）是饮鸩止渴的话，有没有其它的平衡回路可以帮助达到目标？上图左下的平衡回路是一种可能性，当需要加速带来的“压力”变大时，我们增加“提高效率的措施力度”，从而使得“速度”变快，“压力”得以释放，我们把这个平衡回路命名为“提高效率以加速”。提高效率的措施可以有很多，比如通过培训学习提升技能、加强协作、移除障碍等，这些措施相对于走捷径来说通常需要更长的时间才能见效。而当短期内走捷径看起来起作用时，“提高效率的意愿”就会变低，以致于“提高效率的措施力度”变低，“速度”变慢，“压力”变大，进而更多地走捷径，由此形成了一个增强回路，可以命名为“上瘾于走捷径”。“舍本逐末”系统基模的关键特征是短期表面方案和长期根本方案之间的张力，因为增强回路的存在，使得我们上瘾于短期表面方案。
...
1. 走捷径的意图是什么？它想达成什么目标，解决什么问题？达成目标/解决问题可以用平衡回路表示出来，这就是“走捷径以加速”的平衡回路。
1. 走捷径带来什么后果？尤其是拉长时间来看有什么意外的后果。这些后果可能回过来影响之前的目标，形成“缺陷拖后腿”的增强回路，“走捷径以加速”和“缺陷拖后腿”两者构成了“饮鸩止渴”的系统基模。这些后果也可能影响其它方面的目标，比如质量。假设通过走捷径速度确实变快，但同时质量下降，那样的话就是两个系统目标（速度和质量）哪个更重要的问题了。
...
这两个问题跟常用的力场分析法（Force Field Analysis）有些类似，不同的是我们会去寻找可以借用的增强回路和识别需要削弱的平衡回路，而力场分析法往往止步于线性因果。
...
这一章是给后续章节基于系统思考的分析做个准备。系统思考是个专门的学科，有很多书可以参考学习，所以本章只是结合产品开发这个领域的例子来阐述了我们会应用到的基础知识。
